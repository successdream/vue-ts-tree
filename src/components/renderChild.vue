<template>
  <div>
    <div  v-for="(item,index) in data" :key="index" style='padding-left:25px' v-show='item.parentId===currentParentId'>
      <div style="position:relative;" @click="expandIt" :key='item.Id'  :currentid='item.Id'>
        <div  style="display:flex;flex-wrap:wrap;padding:5px 0px 5px 25px;">
          <div ref='triangle' :class='{triangle:true,"rotate-90-deg": allID[item.Id]==item.Id}' v-if='item.children'></div>
...
export default class renderChild extends Vue {
  ...
  expandIt(e:any){
    ...
    if(this.currentIds[targetId] === targetId-0){
      ...
      this.currentIds[targetId] = targetId-0; 
      this.closeAll = true;
    }
